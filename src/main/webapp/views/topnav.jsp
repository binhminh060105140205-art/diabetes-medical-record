<%@taglib prefix="c" uri="jakarta.tags.core"%><%@page contentType="text/html" pageEncoding="UTF-8"%>
<c:set var="uri" value="${pageContext.request.requestURI}"/>
<aside class="app-sidebar" id="app-sidebar">
    <div class="sidebar-context">
        <span class="sidebar-avatar"><c:out value="${sessionScope.user.fullName.substring(0,1)}"/></span>
        <div><strong><c:out value="${sessionScope.user.fullName}"/></strong><small>${sessionScope.user.role}</small></div>
    </div>
    <div class="sidebar-label">KHÔNG GIAN LÀM VIỆC</div>
    <nav class="sidebar-nav"><c:choose>
        <c:when test="${sessionScope.user.role=='ADMIN'}">
            ...
            <a class="${uri.contains('ClinicWorkflow')?'active':''}" href="${pageContext.request.contextPath}/ClinicWorkflow?view=appointments"><span>▦</span><b>Điều hành lịch khám</b></a>
        </c:when>
        <c:when test="${sessionScope.user.role=='STAFF'}">
            ...
            <a class="${uri.contains('ClinicWorkflow')?'active':''}" href="${pageContext.request.contextPath}/ClinicWorkflow"><span>▦</span><b>Lịch hẹn & hàng đợi</b></a>
        ...
        <c:when test="${sessionScope.user.role=='DOCTOR'}">
            ...
            <a class="${uri.contains('ClinicWorkflow')?'active':''}" href="${pageContext.request.contextPath}/ClinicWorkflow"><span>▤</span><b>Lượt khám của tôi</b></a>
        ...
        <c:when test="${sessionScope.user.role=='PATIENT'}">
            ...
            <a class="${uri.contains('PatientAppointments')?'active':''}" href="${pageContext.request.contextPath}/PatientAppointments"><span>▦</span><b>Lịch hẹn</b></a>
    ...
    <div class="sidebar-bottom">
        <a class="sidebar-main-link" href="${pageContext.request.contextPath}/"><span>⌂</span><b>Về trang chính</b></a>
