...
public class ClinicWorkflowDAO extends DBContext implements vn.diabetes.service.ClinicWorkflowGateway {
    ...
    private void inTransaction(String failureMessage, TransactionWork work) {
        inTransaction(failureMessage, null, work);
    }

    private void inTransaction(String failureMessage, String conflictMessage, TransactionWork work) {
        try {
            ...
            if (conflictMessage != null && isUniqueViolation(error)) {
    ...
    private boolean isUniqueViolation(Throwable error) {
        ...
        while (current != null) {
            if (current instanceof SQLException sqlError && super.isUniqueViolation(sqlError)) {
                return true;
            }
    ...
    public PatientAppointmentPageData loadPatientAppointmentPage(int userId) {
        String sql = """
          ...
          SELECT s.patient_id,a.appointment_id,a.appointment_at,
           ...
           FROM subject s OUTER APPLY (
             SELECT TOP 30 * FROM appointments WHERE patient_id=s.patient_id
             ORDER BY preferred_date DESC,
                      CASE WHEN appointment_at IS NULL THEN 0 ELSE 1 END,
                      appointment_at DESC
           ) a
           LEFT JOIN doctors d ON d.doctor_id=a.doctor_id LEFT JOIN users u ON u.user_id=d.user_id
           ORDER BY a.preferred_date DESC,
                    CASE WHEN a.appointment_at IS NULL THEN 1 ELSE 0 END,
            ...
            try (ResultSet rows = ps.executeQuery()) {
                while (rows.next()) {
                    ...
                    if (rows.getObject("appointment_id") != null) {
                        ...
                        appointment.put("appointment_at", rows.getTimestamp("appointment_at"));
                        appointment.put("preferred_date", rows.getDate("preferred_date"));
    ...
    public List<Map<String,Object>> assignedAppointmentsForDoctor(int doctorId) {
        return query("""
                SELECT TOP 30 a.appointment_id,a.appointment_at,a.preferred_date,a.preferred_period,
                ...
                WHERE a.doctor_id=?
                  ...
                  AND (a.appointment_at IS NULL OR a.appointment_at>=CAST(SYSDATETIME() AS DATE))
                ORDER BY CASE WHEN a.appointment_at IS NULL THEN 1 ELSE 0 END,
                         a.appointment_at,a.preferred_date,a.appointment_id""", doctorId);
    }
    ...
    public AppointmentOperationsPageData loadAppointmentOperationsPage() {
        String sql = """
          ...
          SELECT 'PATIENT' row_type,p.patient_id,CAST(NULL AS INT) doctor_id,p.full_name display_name,
                 ...
                 COALESCE(dp.diabetes_type,'UNKNOWN') diabetes_type,
                 CAST(NULL AS INT) appointment_id,CAST(NULL AS DATETIME2) appointment_at,
                 CAST(NULL AS DATE) preferred_date,CAST(NULL AS NVARCHAR(20)) preferred_period,
                 CAST(NULL AS NVARCHAR(255)) reason,CAST(NULL AS NVARCHAR(500)) note,
                 CAST(NULL AS NVARCHAR(30)) status,
                 CAST(NULL AS NVARCHAR(100)) patient_name,CAST(NULL AS NVARCHAR(15)) patient_phone,
                 CAST(NULL AS NVARCHAR(100)) doctor_name,
                 CAST(NULL AS INT) sort_priority,p.created_at sort_date
          ...
          SELECT 'DOCTOR',NULL,d.doctor_id,u.full_name,NULL,d.specialty,d.diabetes_focus,
                 CAST(NULL AS NVARCHAR(10)),NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,
                 CAST(NULL AS INT),CAST(NULL AS DATETIME2)
          ...
          SELECT 'APPOINTMENT',a.patient_id,a.doctor_id,NULL,p.phone,NULL,NULL,
                 ...
                 CASE WHEN a.status='REQUESTED' THEN a.created_at
                      ELSE COALESCE(a.appointment_at,CAST(a.preferred_date AS DATETIME2)) END
           FROM (
             SELECT * FROM appointments WHERE status='REQUESTED'
             UNION ALL
             SELECT recent.* FROM (
               SELECT TOP 50 * FROM appointments WHERE status<>'REQUESTED'
               ORDER BY COALESCE(appointment_at,CAST(preferred_date AS DATETIME2)) DESC
             ) recent
    ...
    public void createAppointmentRequest(int patientId, LocalDate preferredDate,
        ...
        inTransaction("Không thể gửi yêu cầu khám.", () -> {
            ...
            Map<String,Object> requestCapacity = query("""
                    SELECT COALESCE(SUM(CASE WHEN preferred_date=? THEN 1 ELSE 0 END),0) same_day,
                           COALESCE(SUM(CASE WHEN preferred_date>=CAST(SYSDATETIME() AS DATE) THEN 1 ELSE 0 END),0) active_future
                    FROM appointments
                    WHERE patient_id=?
    ...
    public void assignAppointmentRequest(int appointmentId, int doctorId,
        ...
        inTransaction("Không thể xác nhận yêu cầu khám.",
                "Khung giờ này vừa được đặt cho bác sĩ. Vui lòng chọn giờ khác.", () -> {
            List<Map<String,Object>> rows = query("""
                    SELECT patient_id,preferred_date,preferred_period,status
                    FROM appointments WITH (UPDLOCK,HOLDLOCK) WHERE appointment_id=?""", appointmentId);
    ...
    public void rescheduleAppointment(int appointmentId, LocalDateTime newTime,
        ...
        inTransaction("Không thể đổi lịch hẹn.",
                "Khung giờ này vừa được đặt cho bác sĩ. Vui lòng chọn giờ khác.", () -> {
            List<Map<String, Object>> rows = query("""
                    SELECT patient_id,doctor_id,status FROM appointments WITH (UPDLOCK,HOLDLOCK)
            ...
            String period = AppointmentRules.periodOf(newTime.toLocalTime());
            update("""
                    UPDATE appointments SET appointment_at=?,preferred_date=?,preferred_period=?,
                        status='CONFIRMED',note=CASE WHEN ? IS NULL OR ?='' THEN note ELSE ? END
                    WHERE appointment_id=?""",
                    newTime, java.sql.Date.valueOf(newTime.toLocalDate()), period,
                    note, note, note, appointmentId);
    ...
    private void lockAndValidateCapacity(int patientId, int doctorId, LocalDateTime at,
        ...
        try (PreparedStatement lock = connection.prepareStatement("""
                ...
                CROSS JOIN patients p WITH (UPDLOCK,HOLDLOCK)
        ...
        Map<String, Object> capacity = query("""
                SELECT COALESCE(SUM(CASE WHEN doctor_id=? AND appointment_at=? THEN 1 ELSE 0 END),0) doctor_slot,
                       COALESCE(SUM(CASE WHEN patient_id=? AND preferred_date=? THEN 1 ELSE 0 END),0) patient_day,
                       COALESCE(SUM(CASE WHEN doctor_id=? AND preferred_date=? AND preferred_period=? THEN 1 ELSE 0 END),0) doctor_period,
                       COALESCE(SUM(CASE WHEN doctor_id=? AND preferred_date=? THEN 1 ELSE 0 END),0) doctor_day
                FROM appointments
    ...
    private Map<String, Object> lockAppointment(int appointmentId) {
        List<Map<String, Object>> rows = query(
                "SELECT patient_id,doctor_id,appointment_at,status FROM appointments WITH (UPDLOCK,HOLDLOCK) WHERE appointment_id=?",
                appointmentId);
        if (rows.isEmpty()) {
    ...
    private int insertEncounter(
        ...
        try (PreparedStatement statement = connection.prepareStatement(
            ...
            try (ResultSet rows = statement.getGeneratedKeys()) {
                if (!rows.next()) throw new SQLException("Encounter ID was not returned");
    ...
    private int nextQueueNumber() throws SQLException {
        try (PreparedStatement statement = connection.prepareStatement("""
                ...
                FROM queue_entries WITH (TABLOCKX,HOLDLOCK)
                WHERE queued_at>=CAST(SYSDATETIME() AS DATE)
                  AND queued_at<DATEADD(DAY,1,CAST(SYSDATETIME() AS DATE))""")) {
    ...
    public List<Map<String,Object>> encounters() {
        return query("""
          SELECT TOP 50 e.*,p.full_name patient_name,p.phone patient_phone,u.full_name doctor_name,
                 COALESCE(dp.diabetes_type,'UNKNOWN') diabetes_type,
    ...
    public void setEncounterStatus(int encounterId,String status,int actor) {
        ...
        inTransaction("Không thể cập nhật trạng thái lượt khám.", () -> {
            List<Map<String, Object>> rows = query(
                    "SELECT status FROM encounters WITH (UPDLOCK,HOLDLOCK) WHERE encounter_id=?", encounterId);
    ...
    static boolean validEncounterTransition(String current, String target) {
        ...
        return switch (target) {
            ...
            case "COMPLETED" -> "IN_CONSULTATION".equals(current);
    ...
    public void addAllergy(
        ...
        try {
            int changed = update("""
                    ...
                    SET reaction=?,severity=?,status='ACTIVE',noted_by=?,noted_at=CURRENT_TIMESTAMP
                    WHERE patient_id=? AND allergen=?""",
                    reaction, severity, actor, patientId, allergen);
            if (changed == 0) {
                update("""
    ...
    public List<Map<String, Object>> labImportRecords() {
        return query("""
                SELECT TOP 50 m.record_id,p.full_name patient_name,
                        COALESCE(dp.diabetes_type,'UNKNOWN') diabetes_type,
                        STRING_AGG(l.test_code, ', ') WITHIN GROUP (ORDER BY l.test_code) pending_tests
                FROM lab_orders l
    ...
    public void createLabOrder(
        ...
        inTransaction("Không thể tạo chỉ định xét nghiệm.", () -> {
            List<Map<String, Object>> encounters = query("""
                    SELECT patient_id,status FROM encounters WITH (UPDLOCK,HOLDLOCK)
                    WHERE encounter_id=? AND doctor_id=?""", encounterId, doctorId);
            ...
            if (!query("""
                    ...
                    WHERE encounter_id=? AND test_code=? AND status<>'CANCELLED'""",
    ...
    private void saveImportedHealthIndicators(LabResultImportRow row, int actor)
        ...
        try (PreparedStatement statement = connection.prepareStatement("""
                ...
                WHERE record_id=?""")) {
            statement.setInt(1, actor);
            setImportNumber(statement, 2, row.bloodGlucose());
            setImportNumber(statement, 3, row.hba1c());
            setImportNumber(statement, 4, row.cholesterol());
            setImportNumber(statement, 5, row.triglyceride());
            setImportNumber(statement, 6, row.hdlC());
            setImportNumber(statement, 7, row.ldlC());
            statement.setInt(8, row.recordId());
    ...
    public boolean doctorHasPatient(int doctorId, int patientId) {
        return !query(
                "SELECT TOP 1 1 ok FROM encounters WHERE doctor_id=? AND patient_id=?",
