...
## Module vận hành phòng khám

Luồng ngoại trú gồm: đặt lịch, check-in và cấp số hàng đợi, nhập sinh hiệu, bác sĩ khám, chỉ định/trả kết quả xét nghiệm, lưu dị ứng và tiền sử, sau đó kết luận lượt khám. ADMIN, STAFF và DOCTOR mở mục **Điều hành khám** trên thanh điều hướng.

Nếu dùng API nhận dữ liệu thiết bị, cấu hình secret `DEVICE_API_KEY` trên Render và gửi qua header `X-Device-Key`. Không lưu giá trị thật trong Git.
